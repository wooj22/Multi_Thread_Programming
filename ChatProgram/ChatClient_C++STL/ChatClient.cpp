...
// thread
std::thread* g_hSendThread = nullptr;
std::thread* g_hRecvThread = nullptr;
...
// mutex
std::mutex g_csSendQueue;
std::mutex g_csRecvQueue;
...
struct SendThread
{
	void operator()()
	{
		...
		while (g_bRunning)
		{
			...
			std::unique_lock<std::mutex> lock(g_csSendQueue);
			...
			while (!tempQueue.empty())
			{
				...
				if (nSent == SOCKET_ERROR)
				{
					...
					std::unique_lock<std::mutex> lock(g_csRecvQueue);
	...
	case WM_COMMAND:
	{
		...
		case IDC_BTN_CONNECT:
		{
			...
			if (ConnectToServer(szIP, SERVER_PORT)) // 포트는 임의로 11021로 지정
			{
				...
				g_hSendThread = new std::thread(SendThread());
				g_hRecvThread = new std::thread(RecvThread());
...
void CreateUI(HWND hWnd)
{
	...
	g_hEditIP = CreateWindowW(L"EDIT", L"172.21.1.19", WS_CHILD | WS_VISIBLE | WS_BORDER | ES_AUTOHSCROLL, x + 65, y, 120, 20, hWnd, (HMENU)IDC_EDIT_IP, g_hInst, NULL);
...
void DisconnectFromServer()
{
	...
	{
		g_hRecvThread->join();
		g_hRecvThread = nullptr;;
	}
	if (g_hSendThread)
	{
		g_hSendThread->join();
		g_hSendThread = nullptr;;
	}
	...
	std::unique_lock<std::mutex> lock2(g_csRecvQueue);
